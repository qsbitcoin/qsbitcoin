...
RPCHelpMan getnewquantumaddress()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            std::string address = EncodeDestination(*op_dest);
            // Add quantum prefix based on type
            if (scheme_id == quantum::SCHEME_ML_DSA_65) {
                address = "Q1" + address.substr(1);  // Replace first char with Q1
            } else {
                address = "Q2" + address.substr(1);  // Replace first char with Q2
            }
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (isValid) {
                ...
                if (addr_str.length() > 2 && addr_str[0] == 'Q') {
                    // Quantum address
