...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // Check quantum signature policy (e.g., max number of quantum signatures)
    if (m_pool.m_opts.require_standard) {
        std::string quantum_reason;
        if (!quantum::CheckQuantumSignaturePolicy(tx, quantum_reason)) {
            return state.Invalid(TxValidationResult::TX_NOT_STANDARD, 
                               "quantum-signature-policy", quantum_reason);
        }
    }
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // Enforce Quantum-Safe Signatures
    if (DeploymentActiveAt(block_index, chainman, Consensus::DEPLOYMENT_QUANTUM_SIGS)) {
