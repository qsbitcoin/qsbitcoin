...
bool IsStandardTx(const CTransaction& tx, const std::optional<unsigned>& max_datacarrier_bytes, bool permit_bare_multisig, const CFeeRate& dust_relay_fee, std::string& reason)
{
    ...
    // Check weight limits based on whether transaction has quantum signatures
    if (!quantum::IsStandardTxWeight(sz, quantum::HasQuantumSignatures(tx))) {
        reason = "tx-size";
        return false;
    }
    ...
    for (const CTxIn& txin : tx.vin)
    {
        ...
        //
        // For quantum signatures, we allow larger scriptSigs as they can be
        // up to 50KB for SLH-DSA signatures.
        size_t max_scriptsig_size = MAX_STANDARD_SCRIPTSIG_SIZE;
        
        // Check if this input might contain a quantum signature
        if (txin.scriptSig.size() > MAX_STANDARD_SCRIPTSIG_SIZE) {
            // Parse the scriptSig to check for quantum signatures
            CScript::const_iterator pc = txin.scriptSig.begin();
            std::vector<unsigned char> data;
            opcodetype opcode;
            bool has_quantum_sig = false;
            ...
            if (has_quantum_sig) {
                // Allow up to MAX_STANDARD_QUANTUM_SIG_SIZE for quantum signatures
                max_scriptsig_size = quantum::MAX_STANDARD_QUANTUM_SIG_SIZE;
            }
        }
        
        if (txin.scriptSig.size() > max_scriptsig_size) {
